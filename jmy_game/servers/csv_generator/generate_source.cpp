...
bool GenerateSource::init(const char* file_name)
{
	...
	out_file_ << "#include \"../../libjmy/jmy_csv_parser.h\"" << std::endl;
	out_file_ << "#include <iostream>" << std::endl;
	out_file_ << "#include <vector>" << std::endl;

	JmyCsvParser::lines_type& lines = parser_.getLines();
...
bool GenerateSource::writeClassBody(const JmyCsvParser::lines_type& lines)
{
	...
	out_file_ << std::endl;
	out_file_ << "  JmyObjectSet<" << file_name_ << ", " << type_line[key_index_] << "> objs_;" << std::endl;
	out_file_ << "};" << std::endl;
	return true;
}
...
bool GenerateSource::writeLoadFunc()
{
	...
	out_file_ << "    JmyCsvParser parser;" << std::endl;
...
bool GenerateSource::writeParseDataFunc(const JmyCsvParser::lines_type& lines)
{
	out_file_ << "  bool parseData(const JmyCsvParser::lines_type& lines) {" << std::endl;
	out_file_ << "    size_t s = lines.size();" << std::endl;
	...
	for (; field_index<type_line.size(); ++field_index) {
		if (type_line[field_index] == "int") {
			out_file_ << "      data." << name_line[field_index] << " = std::atoi(line[" << field_index << "].c_str());" << std::endl;
		} else if (type_line[field_index] == "int64") {
			out_file_ << "      data." << name_line[field_index] << " = (int64_t)std::strtoull(line[" << field_index << "].c_str(), nullptr, 10);" << std::endl;
		} else if (type_line[field_index] == "string") {
			out_file_ << "      data." << name_line[field_index] << " = line[" << field_index << "];" << std::endl;
		} else if (type_line[field_index] == "bool") {
			out_file_ << "      data." << name_line[field_index] << " = (bool)std::atoi(line[" << field_index << "].c_str());" << std::endl;
		} else {
...
bool GenerateSource::writeCloseFunc(const JmyCsvParser::lines_type& lines)
{
...
bool GenerateSource::writeGetFunc(const std::vector<std::string>& type_line)
{
	...
	out_file_ << "    return objs_.getByKey(key);" << std::endl;
	out_file_ << "  }" << std::endl;
	out_file_ << std::endl;
	out_file_ << "  " << file_name_ << "* getByIndex(uint32_t index) {" << std::endl;
	out_file_ << "    return objs_.getByIndex(index);" << std::endl;
	out_file_ << "  }" << std::endl;
	return true;
}

bool GenerateSource::writeCountFunc()
{
	out_file_ << "  size_t count() const {" << std::endl;
	out_file_ << "    return objs_.count();" << std::endl;
	out_file_ << "  }" << std::endl;
	return true;
}
