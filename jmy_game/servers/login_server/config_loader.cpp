...
bool ConfigLoader::loadJson(const char* jsonpath)
{
	...
	if (!in.is_open()) {
		LogError("failed to open %s", jsonpath);
		return false;
	}
	...
	if (doc_.HasParseError()) {
		LogError("parse %s failed, err %d", jsonpath, doc_.GetParseError());
		return false;
	}

	// server_name
	if (!doc_["server_name"].IsString()) {
		LogError("server_name type is not string");
		return false;
	}
	...
	if (!doc_["server_id"].IsInt()) {
		LogError("server_id type is not int");
		return false;
	}
	...
	if (!doc_["server_ip"].IsString()) {
		LogError("server_ip type is not string");
		return false;
	}
	...
	if (!doc_["server_port"].IsInt()) {
		LogError("server_port type is not int");
		return false;
	}
	...
	if (!doc_["max_conn"].IsInt()) {
		LogError("max_conn type is not int");
		return false;
	}
	...
	if (!doc_["enable_reconnect"].IsBool()) {
		LogError("enable_reconnect type is not bool");
		return false;
	}
	...
	if (!doc_["listen_gate_ip"].IsString()) {
		LogError("listen_gate_ip type is not string");
		return false;
	}
	...
	if (!doc_["listen_gate_port"].IsInt()) {
		LogError("listen_gate_port type is not int");
		return false;
	}
	...
	if (!doc_["listen_gate_max_conn"].IsInt()) {
		LogError("listen_gate_max_conn type is not int");
		return false;
	}
	...
	if (!doc_["listen_gate_enable_reconnect"].IsBool()) {
		LogError("listen_gate_enable_reconnect type is not bool");
		return false;
	}
	...
	if (!doc_["connect_config_ip"].IsString()) {
		LogError("connect_config_ip type is string");
		return false;
	}
	...
	if (!doc_["connect_config_port"].IsInt()) {
		LogError("connect_config_port type is int");
		return false;
	}
	...
	if (!doc_["connect_config_enable_reconnect"].IsBool()) {
		LogError("connect_config_enable_reconnect type is bool");
		return false;
	}
